...
public class AudioManager : MonoBehaviour
{
    ...
    private bool isbgAudio = true;
    public bool IsBgAudioEnable
    {
        get
        {
            return isbgAudio;
    ...
    private Dictionary<string,int> androidClips = new Dictionary<string, int>();
    private void Awake()
    {
        Instance = this;
        ...
        {
            androidClips.Add(clips[i].name, AndroidNativeAudio.load(clips[i].name + ".wav"));
        }
        #endif
    }
...
#if UNITY_ANDROID && !UNITY_EDITOR
        if (androidClips.ContainsKey(audioType.ToString()))
        {
            int SoundID = AndroidNativeAudio.play(androidClips[audioType.ToString()]);
            AndroidNativeAudio.setLoop(SoundID, 0);
        }
#else
        for (int i = 0; i < clips.Count; i++)
        {
